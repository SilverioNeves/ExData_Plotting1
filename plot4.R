...
## Format variables "Global_active_pwer", "Global_reactive_power","Voltage",
## "Sub_metering_1" and "Sub_metering_2" as numeric
datasetf$Global_active_power <- as.numeric(levels(datasetf$Global_active_power))[datasetf$Global_active_power]
datasetf$Global_reactive_power <- as.numeric(levels(datasetf$Global_reactive_power))[datasetf$Global_reactive_power]
datasetf$Voltage <- as.numeric(levels(datasetf$Voltage))[datasetf$Voltage]
datasetf$Sub_metering_1 <- as.numeric(levels(datasetf$Sub_metering_1))[datasetf$Sub_metering_1]
datasetf$Sub_metering_2 <- as.numeric(levels(datasetf$Sub_metering_2))[datasetf$Sub_metering_2]


## Open device and do 4 plots
png("plot4.png", width = 480, height = 480, units = "px", bg = "white")

par(mfrow = c(2,2))

## Plot (1,1) - X labels are in Portuguese
...
## Plot (1,2) - X labels are in Portuguese
with(datasetf, plot(Time,Voltage, type = "l", col="black", bg = "white",xlab = "", ylab = ""))
title(xlab = "Date/Time", ylab = "Voltage")

## Plot (2,1) - X labels are in Portuguese
with(datasetf, plot(Time,Sub_metering_1, type = "l", col="black", bg = "white",xlab = "", ylab = ""))
with(datasetf, lines(Time,Sub_metering_2, type = "l", col = "red", bg = "white"))
with(datasetf, lines(Time,Sub_metering_3, type = "l", col = "blue", bg = "white"))
legend("topright", bty = "n", lty = 1, col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
title(xlab = "", ylab = "Energy sub metering")

## Plot (2,2) -X labels are in Portuguese
with(datasetf, plot(Time,Global_reactive_power, type = "l", col="black", bg = "white",xlab = "", ylab = ""))
title(xlab = "Date/Time", ylab = "Global reactive power")
